...
INSERT INTO `project` VALUES (1,'Remove Counters',2,1,1401236075,1402236139,'This is the first project ever!','1'),(2,'Clean Closet',3,2,1401236075,1403236182,'This is the second project!','1');
...
INSERT INTO `rank` VALUES (1,'Admin'),(2,'Project Manager'),(3,'Task Worker');
...
INSERT INTO `status` VALUES (1,'Open'),(2,'Completed'),(3,'Completed - Pending'),(4,'Closed');
...
/*!50001 VIEW `theprojects` AS select `p`.`project_name` AS `project_name`,`p`.`start_date` AS `start_date`,`p`.`end_date` AS `end_date`,`p`.`description` AS `description`,concat(`u`.`first_name`,' ',`u`.`last_name`) AS `name`,`s`.`status_title` AS `status`,`u`.`user_id` AS `user_id` from ((`project` `p` join `user` `u` on((`p`.`user_id` = `u`.`user_id`))) join `status` `s` on((`p`.`status_id` = `s`.`status_id`))) */;
...
-- Dump completed on 2014-06-10 16:01:45
